...
class PickingManagement extends Component
{
    ...
    public function saveOrder()
    {
        ...
        $this->validate([
            ...
            'order_items.*.product_id.required' => __('You must scan or enter a valid product barcode.'),
            'order_items.*.product_id.exists' => __('The scanned product barcode is invalid.'),
        ]);
    ...
    public function savePick()
    {
        ...
        foreach ($this->pick_items as $index => $pItem) {
            if (empty($pItem['selected_location_id']) && ! empty($pItem['location_barcode'])) {
                $this->resolveLocation($index);
                $pItem = $this->pick_items[$index];
            }
        }
